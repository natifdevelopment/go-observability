...
// MaskRecord masks sensitive attrs in a slog.Record.
// Returns a new record with masked attrs (the original is not modified).
func (m *MaskEngine) MaskRecord(record slog.Record) slog.Record {
	if !m.config.Enabled {
		return record
	}
	// Collect attrs, mask them, then build a new record.
	var attrs []slog.Attr
	record.Attrs(func(attr slog.Attr) bool {
		attrs = append(attrs, attr)
		return true
	})
	masked := m.MaskAttrs(attrs)
	// Build a new record with the masked attrs.
